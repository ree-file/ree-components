...
export interface BodyProps<RecordType> {
  ...
  scrollY: number | null; // 需要滚动高度来定义撑开元素的高度
}
...
function Body<RecordType>({
  ...
  const { fixHeader, horizonScroll, flattenColumns, componentWidth, rowHeight } = React.useContext(
    BodyContext,
  );
  ...
  return React.useMemo(() => {
    ...
    const leftHeight = !!scrollY ? scrollY - data.length * rowHeight : 0;
    const openHeight: number = leftHeight > 0 ? leftHeight : 0;
    return (
      <WrapperComponent className={`${prefixCls}-tbody`}>
        ...
        {rows}
        {scrollY && <div style={{ height: openHeight }} />}
      </WrapperComponent>
    );
  }, [
    data,
