...
// Used for customize scroll 自定义滚动
const EMPTY_SCROLL_TARGET = {};

export const INTERNAL_HOOKS = 'rc-table-internal-hook';

// 确定每一行的高度。真实决定高度的是在样式文件里面,注入到body上下文
const rowHeight = 22;
...
const MemoTableContent = React.memo<MemoTableContentProps>(
  ...
  (prev, next) => {
    if (!shallowEqual(prev.props, next.props)) {
      // shallowEqual通过比较属性里面的值来判断对象是否改变
      return false;
    }

    // No additional（额外的） render when pinged status change.
    // This is not a bug.
...
function Table<RecordType extends DefaultRecordType>(props: TableProps<RecordType>) {
  ...
  // 计算撑开高度
  const scrollHeight: number | null = React.useMemo(() => {
    if (!scroll || !scroll.y) {
      return null;
    }
    if (typeof scroll.y === 'string') {
      return parseInt(scroll.y, 10);
    }
    return scroll.y;
  }, [scroll]);
  // Body
